<script>
	...
	const { scaleLinear, max, scaleBand, axisBottom, axisLeft, stackOrderNone, stackOffsetNone } =
		pkg;
	...
	const width = browser
		? Math.max(Math.min(window.innerWidth, 750), 300) - margin.left - margin.right - 10
		: 1000;
	...
	async function drawGraph() {
		...
		const keys = [
			...new Set(
				data.reduce(
					(acc, val) => [
						...acc,
						...Object.keys(val).filter((key) => !getXVal({ [key]: 42 }) && key !== 'count')
					],
					[]
				)
			)
		...
		var stack = d3Stack().keys(keys).order(stackOrderNone).offset(stackOffsetNone);
		...
		const y = scaleBand().domain(data.map(getXVal)).range([0, heightContent]).padding(0.15);

		// chart
		const svg = d3Select(`#chart-${randomNumber}`)
			.html('')
			.attr('style', `width: ${width + margin.left}px;` + `position: relative;`)
			.append('div')
			.attr(
				'style',
				`height: ${height}px;` +
		...
		legend
			...
			.attr(
				'transform',
				(d, i) => 'translate(' + (legendCellSize + 5) + ', ' + i * legendCellSize + ')'
			)
