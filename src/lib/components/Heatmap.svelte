<script>
	...
	$: documentId, acteurId, drawGraph();
	onMount(drawGraph);

	async function drawGraph() {
		if (!browser) return;

		days = range(31).map((v) => v + 1);
		width = Math.max(Math.min(window.innerWidth, 600), 300) - margin.left - margin.right - 10;
		gridSize = {
		...
		if (processedData.length) {
			initColorScale(processedData.reduce((acc, val) => [...acc, ...val.data], []));
			processedData.forEach((v) => drawYear(v.year, v.data));
			drawColorScale(processedData.reduce((acc, val) => [...acc, ...val.data], []));
		}
	}
	...
	function formatNumber(number) {
		return number
			...
			.join('') // reverse number
			.match(/\d{1,3}/g)
			.join(' ') // add spaces every 3 digits
			.split('')
			.reverse()
			.join(''); // reverse number
	}
...
<!-- Graph wrapper is not "if encapsulated" since it disallows d3 to find it on drawing -->
<div id="heat-map" />
