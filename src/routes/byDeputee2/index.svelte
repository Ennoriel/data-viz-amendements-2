<script>
	...
	$activeRoute = routes.find((route) => route.href === '/byDeputee2');
	...
	onMount(loadGraph);
	// $: documentIds, acteurIds, loadGraph()
	...
	async function loadGraph() {
		...
		rawData.links = await send(fetch, '/api/graph/sankey.json', { documentIds, acteurIds });
		...
		svg.style('background', '#fff').attr('width', width).attr('height', height);
...
<select bind:value={acteurIds} multiple>
	{#if $ref.acteurs}
		{#each $ref.acteurs as refActeurs}
			<option value={refActeurs.uid}
				>{`${refActeurs.prenom} ${refActeurs.nom} (${refActeurs.groupe})`}</option
			>
		{/each}
	{/if}
</select>
